...
def dataset_loader(dataset):
    ...
    if dataset in DATASETS:
        ...
        return X, cat_x, int_x, y, bin_y, cat_y, int_y
...
def fetch_higgs():
    ...
    with open('datasets/higgs/HIGGS.csv.gz', 'rb') as f:
        df = pd.read_csv(f, delimiter=',', header = 0)
        Xy = {}
        Xy['data'] = df.values[:, 1:].astype('float')
...
#     def loader(self):
#             bin_y = False # binary outcome
#             cat_y = True # categorical w/ >=2 outcome
#             int_y = False # integer outcome
#             # bin_x = None # binary
#             cat_x = None # categorical w/ >=2 classes
#             int_x = None # integers
...
#                 y= np.array([  1 if np.sum(row)>0 else 0 for row in data_]  )
#             elif self.dt_name== "Maj_vote>0":
#                 y= np.array([  1 if sum(row>0)>= col_num//2 else 0 for row in data_]  )
#             elif self.dt_name== "XORofAllSign":
#                 y=np.array([ XOR_of_signs(row)  for row in data_] )
#             else:
#                 raise Exception("Choose your own experiment")
#             # Xy= copy.deepcopy(np.concatenate((data_,y.reshape(-1,1)), axis=1))
#             data_[data_ == -1] = 0
#             mask=[True]*cat_var_nb+[False]*cont_var_nb+[True]
#             cat_x=[i for i in range(len(mask)-1) if mask[i]]
#             return data_, cat_x, int_x, y, bin_y, cat_y, int_y

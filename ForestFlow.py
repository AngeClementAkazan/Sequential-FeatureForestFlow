...
# Euler solver
def euler_solve(my_model,y0, N=101):
  h = 1 / (N-1)
  y = y0
  t = 0
  # from t=0 to t=1
...
class ForestDiffusionModel():
  ...
  def generate(self, batch_size=None, n_t=None, X_covs=None):
    ...
    # normal_distribution = np.random.normal(loc=0.1, scale=1.1,size=(self.b if batch_size is None else batch_size, self.c))
    # normal_distribution = np.random.normal(loc=0, scale=1, size=1000)
    # Add uniform noise to the normal distribution
    # uni = np.random.uniform(low=-0.1, high=0.1, size=(self.b if batch_size is None else batch_size, self.c))
    # y0= normal_distribution 

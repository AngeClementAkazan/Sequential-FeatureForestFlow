...
class TestTrainingClass(unittest.TestCase):
    def test_training_method(self):
        ...
        Metrics4_data={}
        ngen,nexp=1,1
        N,K_dpl,Which_solver,problem_type=2,1,"Euler", ["Class","Class","Class","Class","Reg"]
        for i in range(len(data_set_name)):
            if data_set_name[i]=="iris":
                Metrics4_data[data_set_name[i]]=Metrics(ngen,nexp,sampling,data_loader(data_set_name[i]),data_set_name[i],
                                         N,K_dpl,Which_solver,problem_type[i],forest_flow=False,mask_cat=None)
        Metric_dt=create_csv(data_set_name,Metrics4_data)
        print(Metric_dt)
        #Save it as csv
#         Metric_dt.to_csv('/Users/ange-clementakazan/Documents/Metrics_4_Forest_Flow_Based_Variable_Sampling.csv')
